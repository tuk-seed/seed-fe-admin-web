...
const TransactionCard = ({ customerName, customerId, amount, date, time }) => {
  ...
  return (
   <div className="rounded-lg p-0 flex items-center justify-between border-b">
    ...
    <div className="h-12 w-17 rounded-full  flex items-center justify-center text-white text-xl">
  ...
  <div className='flex flex-col text-s text-black pl-4'>
    <h2 className="text-sm font-semibold mb-0 text-right p-2">{customerName}</h2>
    <p className="text-gray-600 text-left text-xs pb-2">{customerId}</p>
 </div>
</div>
...
<div className='flex flex-row'>
 <div className='flex justify-end'>  
  ...
  <p className="text-sm font-semibold mb-0 text-right p-2">-KSH{amount}</p>
    <p className="text-gray-600 text-xs pb-2">{formatDateAndTime()}</p>
  </div>
  </div> 
</div>
</div>
